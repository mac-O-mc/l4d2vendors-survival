...
::Clockwork.ThinkWait <- function(seconds, func, args = [], unprotected = false, ID = null, repeat = false, rethinkrate = 0.033)
{
	try
	{
		...
		local ThinkTask = { SecondsToAwait = seconds, Func = func, Args = args, Unprotected = unprotected, Repeat = repeat, RethinkRate = rethinkrate, LastTime = Time() }
			...
			{		
				if(!ThinkTask.Unprotected)
				{
					try
					{
						...
						if( typeof ThinkTask.Args == "array" || typeof ThinkTask.Args == "table" )
							ThinkTask.Args.len() != 0 ? ThinkTask.Func(ThinkTask.Args) : ThinkTask.Func.call(this)
