...
if (!("Flamboyance" in getroottable()))
{
	...
	::Flamboyance.PrintToChatClient <- function(client, string, colorcode = "Orange")
	{
		try{
			ClientPrint(client, DirectorScript.HUD_PRINTTALK, (ParseEnums(colorcode))+string);
		}
		catch(exception)
			ClientPrint(null, DirectorScript.HUD_PRINTTALK, (ParseEnums(colorcode))+"::Flamboyance.PrintToChatClient throwing exception: "+exception);
	}
}